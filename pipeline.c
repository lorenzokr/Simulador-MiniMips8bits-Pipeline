...
int main() {
    ...
    unidade_forwading         entradas_forwarding = {0}; 
    sinais_controle_forwading sinais_forwarding   = {0}; 
            ...
            case 9: 
                ...
                do 
                {
                    ...
                    if (reg_MemWb.instrucao.opcode == 0) {
                        ...
                        metricas.contInst++;
                    } else if(reg_MemWb.instrucao.opcode != -1) { 
                    ...
                    // FLUSH NO REGISTRADOR EX/MEM (Limpa a instrução que acabou de vir do ID)
                    if (desvio_tomado) {
                        if (escolha == 9) printf("    [!] FLUSH: Instrucao no estagio EX descartada devido ao desvio tomado.\n");
                    ...
                    // FLUSH NO REGISTRADOR ID/EX (Limpa a instrução que acabou de vir do IF)
                    if (desvio_tomado) {
                        if (escolha == 9) printf("    [!] FLUSH: Instrucao no estagio ID descartada devido ao desvio tomado.\n");
                    ...
                    // FLUSH NO REGISTRADOR IF/ID (Limpa a instrução que acabou de ser buscada incorretamente)
                    if (desvio_tomado) {
                        if (escolha == 9) printf("    [!] FLUSH: Instrucao no estagio IF descartada devido ao desvio tomado.\n");
                    ...
                    // ATUALIZAÇÃO DO PC 
                    // =========================================================
                    // Se o desvio foi tomado, força a alteração do PC ignorando stalls de hazard anteriores
...
void imprimir_instrucao(instrucao p) {
  if (p.opcode == -1) {
      printf("|Assembly: NOP (Bolha / Pipeline Flush)\n");
      return;
  }
  switch (p.opcode)
    {
...
REG_pepiline_EX_MEM estagio_ex(REG_pepiline_ID_EX id, int ex_mem, int mem_wb, sinais_controle_forwading sinal_forwading)
{
    ...
    resultado_forwadingB = mux_forwadingB(id.saida2_banco_reg, ex_mem, mem_wb, sinal_forwading.forwadingB); 
...
void estagio_wb(REG_pepiline_MEM_WB Mem,int banco_registrador[8])
{
    ...
    if(Mem.sinais_wb.RegWrite && Mem.instrucao.opcode != -1)
    {
